...
RobotInfo = [
  {body: null,  // for MatterJS body, added by InstantiateRobot()
   ...
   sensors: [  // define an array of sensors on the robot
     // define right sensor
     {sense: senseTouch,  // function handle, determines type of sensor
      ...
      id: 'touchR',  // a unique, arbitrary ID of the sensor, for printing/debugging
     ...
     {sense: senseDistance,  // function handle, determines type of sensor
      minVal: 1,  // minimum detectable distance, in pixels
...
simInfo = {
  ...
  airDrag: 0.05,  // "air" friction of enviroment; 0 is vacuum, 0.9 is molasses
  boxFric: 0.005, //
  boxMass: 0.001,  // mass of boxes
...
function rotate(robot, torque=0) {
   ...
   *
   * Parameters
   *   torque - rotational force to apply to the body; try values around 0.001.
   */
...
function drive(robot, force=0) {
   ...
   *
   * Parameters
   *   force - force to apply to the body; try values around 0.001.
   */
...
function senseTouch() {
  ...
  // indicate if the sensor exceeded its maximum length by returning 0
  if (rayLength > this.maxVal) {
    touched = 0;
  }
  else {
...
function InstantiateRobot(robotInfo) {
  ...
  // instantiate weights
  this.weightIn = [1,0,0,1];
  this.weightOu = [1,-1];
  this.learningRate = 0.01;//0.01;
...
function sigmoid(val, shift=6) {
  output = 1 / (1+Math.exp(-(val - shift)));
  return output;
}
...
function robotMove(robot) {
   ...
   // fetch sensor values
   const touchL = getSensorValById(robot, 'touchL'),
         distL = getSensorValById(robot, 'distL'),
         distR = getSensorValById(robot, 'distR'),
         touchR = getSensorValById(robot, 'touchR');

  // make data array, use 1/distance to make is turn faster the closer it is
   var tmpArray = [touchL, 1/distL, 1/distR, touchR];  // data array per sim step

   // compute output activation of left and right node
   nodeL = tmpArray[0]*robot.weightIn[0] + tmpArray[1]*robot.weightIn[1];
   nodeR = tmpArray[2]*robot.weightIn[2] + tmpArray[3]*robot.weightIn[3];

   // update weights with hebb rule
   robot.weightIn[0] = robot.weightIn[0] + robot.learningRate * nodeL * tmpArray[0];
   robot.weightIn[1] = robot.weightIn[1] + robot.learningRate * nodeL * tmpArray[1];
   robot.weightIn[2] = robot.weightIn[2] + robot.learningRate * nodeR * tmpArray[2];
   robot.weightIn[3] = robot.weightIn[3] + robot.learningRate * nodeR * tmpArray[3];

   // determine robot action depending on touch sensor activity
    if (touchL==1) {  // if left sensor is touched rotate to the right
      robot.drive(robot, 0.0001);
      robot.rotate(robot, 0.01);
    }
   else if (touchR==1) {  // if right sensor is touched rotate to the left
      robot.drive(robot, 0.0001);
      robot.rotate(robot, -0.01);
    }
    else{  // if neither are touched use Hebb network to rotate robot
      robot.drive(robot, 0.0001);
      robot.rotate(robot, nodeL - nodeR);
    }
};
