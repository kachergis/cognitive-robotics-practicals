...
RobotInfo = [
  {body: null,  // for MatterJS body, added by InstantiateRobot()
   ...
   sensors: [  // define an array of sensors on the robot
     ...
     {sense: senseDistance,  // function handle, determines type of sensor
      ...
      attachAngle: +Math.PI/10,  // sensor's angle on robot body
      attachRadius: 25, // sensor's distance from robot body center
     ...
     {sense: senseDistance_noBox, minVal: 0, maxVal: 50, attachAngle: +Math.PI/15,
      attachRadius: 20, lookAngle: 0, id: 'dist_noBox', color: [100, 100, 100, 50],
      parent: null, value: null, valueStr: ''
     },
     // define a color sensor
     {sense: senseColor, minVal: 0, maxVal: 10, attachAngle: +Math.PI/10,
      attachRadius: 5, lookAngle: 20, id: 'color', color: [150, 0, 150, 50],
      parent: null, value: null, valueStr: ''
     },
     // use gyroscope/angle sensor
  ...
  {body: null,  // for MatterJS body, added by InstantiateRobot()
   color: [255, 255, 255],  // color of the robot shape
   init: {x: 250, y: 50, angle: 0},  // initial position and orientation
   sensors: [  // define an array of sensors on the robot
     // define one sensor
     {sense: senseDistance,  // function handle, determines type of sensor
      minVal: 0,  // minimum detectable distance, in pixels
      maxVal: 25,  // maximum detectable distance, in pixels
      attachAngle: Math.PI/10,  // sensor's angle on robot body
      attachRadius: 20, // sensor's distance from robot body center
      lookAngle: 0,  // direction the sensor is looking (relative to center-out)
      id: 'dist_all',  // a unique, arbitrary ID of the sensor, for printing/debugging
      color: [0, 0, 0, 50],  // sensor color [in RGBA], to distinguish them
      parent: null,  // robot object the sensor is attached to, added by InstantiateRobot
...
simInfo = {
  maxSteps: 80000,  // maximal number of simulation steps to run
  airDrag: 0.1,  // "air" friction of environment; 0 is vacuum, 0.9 is molasses
  objFric: 0, // friction during collisions between any object (box, robot, wall)
  boxMass: 0.01,  // mass of boxes
  boxSize: 12,  // size of the boxes, in pixels
  robotSize: 100, // approximate robot radius, to select by clicking with mouse
  robotMass: 0.2, // robot mass (a.u)
  ...
  debugLemmings: true,  // use colors to indicate state of robots, see robotMove()
...
function senseDistance(sensor, noBoxes=false) {
  ...
  // ignore own body for more slack/convenience in sensor placement
  const bodies_except_self = [];
  for (bb = 0; bb < bodies.length; bb++) {
    if (bodies[bb] != sensor.parent.body) {
      bodies_except_self.push(bodies[bb]);
    }
  }
  bodies = bodies_except_self;
...
function senseColor(sensor) {
  /* Color sensor simulation based on ray casting. Called from sensor
   * object, returns nothing, updates a new reading into sensor.value.
   *
   * Idea: Cast a ray with a certain length from the sensor, and check
   *       via collision detection if objects intersect with the ray.
   *       To determine color return color of object + some noise which
   *       intersects with the array.
   * Note: Sensor ray needs to ignore robot (parts), or start outside of it.
   *       The latter is easy with the current circular shape of the robots.
   * Note: Order of tests are optimized by starting with max ray length, and
   *       then only testing the maximal number of initially resulting objects.
  ...
  // indicate if the sensor exceeded its maximum range by returning infinity
  var colorBody = [Infinity, Infinity, Infinity];
  if (rayLength <= sensor.maxVal) {
    // apply mild noise on the sensor reading, and clamp between valid values
    function gaussNoise(sigma=1) {
      const x0 = 1.0 - Math.random();
    ...
    function roundClamp(x) {
      return Math.round(Matter.Common.clamp(x, 0, 255));
    }

    const clamp = Matter.Common.clamp;
    if (bodies[0].color) {
      colorBody[0] = bodies[0].color[0] + gaussNoise(5);
      colorBody[1] = bodies[0].color[1] + gaussNoise(5);
      colorBody[2] = bodies[0].color[2] + gaussNoise(5);
      colorBody[0] = roundClamp(colorBody[0]);
      colorBody[1] = roundClamp(colorBody[1]);
      colorBody[2] = roundClamp(colorBody[2]);
    }
  }

  sensor.value = colorBody;  // for using in program, e.g. robotMove()
  sensor.valueStr = '' + colorBody;  // for printing to screen
};
...
function getSensorById(robot, sensor_id) {
  for (var ss = 0; ss < robot.sensors.length; ss++) {
    if (robot.sensors[ss].id == sensor_id) {
      return robot.sensors[ss];
    }
  }
  return undefined;  // if not returned yet, sensor_id doesn't exist
};
...
function rgb2ColorStr(rgb_array) {
  // Convert raw RGB(A) color array into some relevant color strings.
  // It simply checks for the "main" color component (red, green, or blue),
  // based on an arbitrary threshold. If the threshold isn't reached for either
  // color channel, the function simply returns 'gray'.
  const threshold = 25,  // arbitrary threshold, to avoid noise
        red = rgb_array[0],
        green = rgb_array[1],
        blue = rgb_array[2];

  if (red > (green+blue)/2 + threshold ) {
    return 'red';
  }
  if (green > (red+blue)/2 + threshold ) {
    return 'green';
  }
  if (blue > (red+green)/2 + threshold ) {
    return 'blue';
  }
  return 'gray';
}

function randomSign(bias=0.5) {
  var rand_sign = 1;
  if (Math.random() > bias) {
    rand_sign = -1;
  }
...
function robotMove(robot) {
  /* Defines Lemming behavior. */

  // Get all relevant sensor objects.
  const seesColor =  rgb2ColorStr(getSensorValById(robot, 'color')),
        snsrDistAll = getSensorById(robot, 'dist_all'),
        snsrDistNoBox = getSensorById(robot, 'dist_noBox');

  /* Define relevant states based on these values. */
  // Obstacles are objects other than boxes, i.e. walls or other robots.
  const seesObstacle = snsrDistNoBox.value < snsrDistNoBox.maxVal,
        seesBox = snsrDistAll.value < snsrDistAll.maxVal,
        carriesBlock = seesColor != 'gray',
        seesRed = seesColor == 'red',
        seesBlue = seesColor == 'blue';

  /* Define some constants for fine-tuning the behavior */
  const defaultForce = 0.0001,
        slightTorque = 0.0001,  // can't be too much or driving in circles
        turnTorque = 0.002;

  // for debugging, show what color is read by showing that on robot itself
  if (simInfo.debugLemmings) {
    if (seesRed) {
      robot.body.color = [200, 0, 0];
    }
    else if (seesBlue) {
      robot.body.color = [0, 0, 200];
    }
    else {
      robot.body.color = RobotInfo[0].color;
    }
  }
  ...
  if (seesObstacle) {
    ...
    if (!carriesBlock) {
      robot.move = rotateBySetRadians(robot, randomSign()*Math.PI/3, turnTorque);
    }
    // – If it carries a blue block it should turn left to leave the block.
    else if (seesBlue) {
      robot.move = rotateBySetRadians(robot, -3*Math.PI/2, turnTorque*2);
    }
    // – If it carries a red block it should turn right to keep the block.
    else if (seesRed) {
      robot.move = rotateBySetRadians(robot, +Math.PI/2, turnTorque);
    }
  }
  // If it senses a block its subsequent behavior depends on whether a block is
  // in the gripper and the block color.
  else if (seesBox) {  // not seeing an obstacle
    // – If it carries no block it should drive straight towards it, and thus get
    //   the block into the gripper.
    if (!carriesBlock) {
      robot.drive(robot, defaultForce);
    }
    // – If it carries a blue block it keeps wandering and ignores the other block.
    else if (seesBlue) {
      robot.drive(robot, defaultForce);
      robot.rotate(robot, slightTorque);
    }
    // – If it carries a red block it turns left to leave the carried block.
    else if (seesRed) {
      robot.move = rotateBySetRadians(robot, -3*Math.PI/2, turnTorque*2);
    }
  }
...
function checkIfBoxSortingIsDone() {
  /* Determine status of sorted boxes, and print overview to HTML interface.
   * Returns true if a certain criterion is reached, see at end of function.
   */

  const context = document.getElementById('arenaLemming').getContext('2d'),
        bodies = Matter.Composite.allBodies(simInfo.engine.world),
        margin = 3*simInfo.boxSize,
        boundLeft = margin,
        boundRight = simInfo.width - margin,
        boundTop = margin,
        boundLow = simInfo.height - margin,
        boundMarkColor = convrgb([100, 100, 0, 50]);

  /* Count boxes pushed to a wall versus inside the arena; per color. */
  function boxIsAtWall(bx, by) {
    if (bx < boundLeft) { return true; }
    if (bx > boundRight) { return true; }
    if (by < boundTop) { return true; }
    if (by > boundLow) { return true; }
    return false;
  }

  function markBox(bx, by) {
    context.beginPath();
    context.arc(bx, by, 4, 0, 2*Math.PI);
    context.closePath();
    context.fillStyle = boundMarkColor;
    context.fill();
  }

  var totalBoxesBlue = 0,
      totalBoxesRed = 0,
      atWallBoxesBlue = 0,
      atWallBoxesRed = 0;
  ...
  for (bb = 0; bb < bodies.length; bb++) {
    if (bodies[bb].role == 'box') {
      var box = bodies[bb],
          boxColorStr = rgb2ColorStr(box.color);

      if (boxColorStr == 'red') {
        totalBoxesRed++;
        if (boxIsAtWall(box.position.x, box.position.y)) {
          atWallBoxesRed++;
          if (simInfo.debugLemmings) {
            markBox(box.position.x, box.position.y);
          }
        }
      }
      else if (boxColorStr == 'blue') {
        totalBoxesBlue++;
        if (boxIsAtWall(box.position.x, box.position.y)) {
          atWallBoxesBlue++;
          if (simInfo.debugLemmings) {
  ...
  const atWallPercentRed = Math.round(100 * atWallBoxesRed / totalBoxesRed),
        atWallPercentBlue = Math.round(100 * atWallBoxesBlue / totalBoxesBlue);

  // show relevant numbers in HTML interface
  document.getElementById("BoxSortLabel").innerHTML =
    '<span style="color:red;"> ' + atWallBoxesRed + ' of '
    + totalBoxesRed + ' (' + atWallPercentRed + '%)</span>, '
    + '<span style="color:blue;">' + atWallBoxesBlue + ' of '
    + totalBoxesBlue + ' (' + atWallPercentBlue + '%)</span>.';

  if (simInfo.debugLemmings) {  // draw boundaries used for checks
    context.moveTo(boundLeft, boundTop);
    context.beginPath();
    context.lineTo(boundRight, boundTop);
    context.lineTo(boundRight, boundLow);
    context.lineTo(boundLeft, boundLow);
    context.lineTo(boundLeft, boundTop);
    context.closePath();
    context.strokeStyle = boundMarkColor;
    context.lineWidth = 1;
    context.stroke();
  }

  // Consider a sorting run is done if 80% of the blue boxes
  // located at a wall (sorted away).
  // TODO? Maybe a better one would also check for the red boxes?
  if (atWallPercentBlue > 80)
    return true;
  else {
    return false;
  }
...
function simStep() {
  ...
  if (simInfo.curSteps < simInfo.maxSteps) {
    ...
    // assess box sorting status and end simulation if it's done
    // (call after drawing everything else, as it draws debug info over objects)
    if (checkIfBoxSortingIsDone()) { toggleSimulation(); }
  }
  else {
    toggleSimulation();
  }
  ...
  for (var bb = 0; bb < bodies.length; bb += 1) {
    ...
    if (bodies[bb].role != 'robot') {
      ...
      if (bodies[bb].color) {
        ...
        context.fillStyle = convrgb(bodies[bb].color);
        context.closePath();
        // context.stroke();
        context.fill();
      }
  ...
  if (!(simInfo.curSteps % 5)) {  // update slow enough to read
    ...
    for (ss = 0; ss < rsensors.length; ss++) {
      ...
      sensorString += '<br> <span style="color:' + colorStr + ';">'
        + 'id \'' + sensor.id + '\': ' + sensor.valueStr + '</span>';
    }
